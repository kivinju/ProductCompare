...
public class ManagerController {
	...
	public String synonymManage(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String synonymAdd(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String synonymDelete(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		...
		List<Synonym> synonymlist = (List<Synonym>) session
				.getAttribute("synonymlist");
		for (int i = 0; i < synonymlist.size(); i++) {
			Synonym base = synonymlist.get(i);
			for (int j = 0; j < base.getList().size(); j++) {
				if ((base.getId() == id1)
						&& (base.getList().get(j).getId() == id2)) {
	...
	public String ipManage(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String ipAdd(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String ipDelete(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String ip = request.getParameter("ip");
	...
	public String sensitiveWordManage(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String sensitiveWordAdd(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String word = request.getParameter("word");
		Sensitives sensitives = new Sensitives(word);
		sensitivesService.addSensitives(sensitives);
		List<Sensitives> sensitiveslist = (List<Sensitives>) session
				.getAttribute("sensitiveslist");
	...
	public String sensitiveWordDelete(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String word = request.getParameter("word");
	...
	public String biddingManage(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String biddingAdd(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		int pid = Integer.parseInt(request.getParameter("pid"));
		int money = Integer.parseInt(request.getParameter("money"));
		Bidding bidding = biddingService.addBidding(pid, money);
		List<Bidding> biddinglist = (List<Bidding>) session
				.getAttribute("biddinglist");
		if (bidding != null)
	...
	public String biddingDelete(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		int pid = Integer.parseInt(request.getParameter("pid"));
		int money = Integer.parseInt(request.getParameter("money"));
		...
		for (int i = 0; i < biddinglist.size(); i++) {
			if ((biddinglist.get(i).getProducts().getPid() == pid)
					&& (biddinglist.get(i).getMoney() == money)) {
	...
	public String armyManage(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String armyAdd(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String ip = request.getParameter("ip");
		WaterArmy waterArmy = new WaterArmy(ip);
		armyService.addArmy(waterArmy);
		List<WaterArmy> armylist = (List<WaterArmy>) session
				.getAttribute("armylist");
	...
	public String armyDelete(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String ip = request.getParameter("ip");
	...
	public String interestManage(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
	...
	public String interestAdd(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String word = request.getParameter("word");
		String seller = request.getParameter("seller");
		InterestWord interestWord = new InterestWord(word, seller);
		interestService.addInterestWord(interestWord);
		List<InterestWord> interestlist = (List<InterestWord>) session
				.getAttribute("interestlist");
	...
	public String interestDelete(HttpServletRequest request, Model model,
			HttpServletResponse response, HttpSession session) {
		String word = request.getParameter("word");
		String seller = request.getParameter("seller");
		...
		for (int i = 0; i < interestlist.size(); i++) {
			if (interestlist.get(i).getWord().equals(word)
					&& interestlist.get(i).getSeller().equals(seller)) {
		...
		if (multipartResolver.isMultipart(request)) {
			...
			if (iter.hasNext()) {
				...
				if (file != null) {
					...
					AdsDB db = new AdsDB();
					db.store(file.getBytes(),money,url);
				}
			}
		}
		return "manager/index";
